            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].Suser === userName || data[i].email === userName) {
                    if (data[i].Spassword === pass) {
                        ...
                        // var m = path.join(__dirname, '/static/', 'stuHomePage.html');
                        // res.redirect('../handleLogin/toStuHomePage');
                        check = true;
                    } else {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].Tuser === userName || data[i].Temail === userName) {
                    if (data[i].Tpassword === pass) {
                        res.send('<script>window.location.href="../teacherHomePage.html"</script>').end();
                        // res.sendFile('teacherHomePage.html',{root:path.join( __dirname,'../static')});
                        check = true;
                    } else {
                        res.send('<script>alert("密码错误")</script>').end();
                        check = true;
