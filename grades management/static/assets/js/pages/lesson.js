$(document).ready(function () {
    ...
    var turnPage = function(obj){
        $('.panel-body>.display').empty()
            .append('<thead><tr><th>课程编号</th><th>教师编号</th><th>课程名称</th><th>教室</th><th>学分</th></tr></thead>')
            .append(obj.tBody)
            .append('<tfoot><tr><th>课程编号</th><th>教师编号</th><th>课程名称</th><th>教室</th><th>学分</th></tfoot>')
    ...
    $('#nav-lesson').click(function () {
        ...
        // 给头div添加内容<h4>
        $titleDiv.empty().append('<h4 class="panel-title">课程基本信息</h4>')
        // 添加显示数据条数
        ...
        /*****************创建表结构结束***********************/
        /*****************ajax请求数据开始*********************/
        $.get('/lesson/info', function (data, error) {
            if (!error) {
                console.log(error);
                console.log(2);
            } else {
                ...
                $table.empty()
                    .append('<thead><tr><th>课程编号</th><th>教师编号</th><th>课程名称</th><th>教室</th><th>学分</th></tr></thead>')
                    .append(arrPages[0])
                    .append('<tfoot><tr><th>课程编号</th><th>教师编号</th><th>课程名称</th><th>教室</th><th>学分</th></tr></tfoot>');
                // 写翻页按钮和数据显示条数
                ...
                // var ii = 3;
                var dataShowStr ='<div class="data-show">' + '第 ' + 1 + ' 页：'+ '第 ' + (ii * 0 + 1) + '~' + (ii * 0 + arrPages[0].context.children.length) + ' 条数据' + '</div>';
