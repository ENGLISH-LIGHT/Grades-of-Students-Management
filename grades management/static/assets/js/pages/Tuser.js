$(document).ready(function () {
    ...
    var turnPage = function(obj){
        $('.panel-body>.display').empty()
            .append('<thead><tr><th>编号</th><th>用户名</th><th>密码</th><th>绑定的邮箱</th><th>绑定的电话</th></tr></thead>')
            .append(obj.tBody)
            .append('<tfoot><tr><th>编号</th><th>用户名</th><th>密码</th><th>绑定的邮箱</th><th>绑定的电话</th></tr></tfoot>');
    ...
    $('#nav-Tuser').click(function () {
        ...
        // 给头div添加内容<h4>
        $titleDiv.empty().append('<h4 class="panel-title">老师账号&密码</h4>')
        // 添加显示数据条数
        ...
        /*****************创建表结构结束***********************/
        /*****************ajax请求数据开始*********************/
        $.get('/Tuser/info', function (data, error) {
            if (!error) {
                console.log(error);
                console.log(2);
            } else {
                ...
                var divGroup =function(){
                    ...
                    for (var i = 0; i < data.length; i++ , count++) {
                        ...
                        var strTds = '<td>' + data[i].ID + '</td>'
                            + '<td>' + data[i].Tuser + '</td>'
                            + '<td>' + data[i].Tpassword + '</td>'
                            + '<td>' + data[i].Temail + '</td>'
                            + '<td>' + data[i].Ttel + '</td>';
